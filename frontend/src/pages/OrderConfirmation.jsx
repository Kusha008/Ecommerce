...
function OrderConfirmation() {
  ...
  const checkoutHandler = async (key, orderIdMongoose) => {
    const response = await axios.post(
      "http://localhost:8000/api/v1/payments/createOrder",
      {
        amount: resultant,
    ...
    var options = {
        ...
        currency: "INR",
        name: "EXPEDITE MART",
        description: "Purchase from Expedite Mart",
        image: Logo,
        ...
        try {
          const validateRes = await axios.post(
            "http://localhost:8000/api/v1/payments/verifyPayment",
            body,
            {
              headers: {
    ...
    razor.on("payment.failed", async function (error) {
      ...
      try {
        const res = await axios.post(
          "http://localhost:8000/api/v1/payments/paymentfailure",
          {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/api/v1/payments/getKeys",
        {},
        { withCredentials: true }
      );
