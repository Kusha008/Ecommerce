...
const CategoryPage = () => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const categoryResponse = await axios.get(`http://localhost:8000/api/v1/categories/${categoryId}`, { withCredentials: true });
        console.log('Category response:', categoryResponse.data.data); // Debugging line
        setCategory(categoryResponse.data.data);

        const productsResponse = await axios.get(`http://localhost:8000/api/v1/products/category/${categoryId}`, { withCredentials: true });
        
        console.log('Products response:', productsResponse); // Debugging line
