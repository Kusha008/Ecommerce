...
const updateCoverImageSeller=asyncHandler(async(req,res)=>{
    ...
    const currImage=await seller.sellerImage;
        ...
        try {
            ...
            if(currImage){
                try {
                    ...
                    seller.sellerImage=coverImage.url;
                    await seller.save({validateBeforeSave:false});
                }
                catch (error) {
            ...
            return res.status(200).json(new ApiResponse(200, seller, "Cover image updated successfully"));
        } catch (error) {
...
const deleteSeller=asyncHandler(async(req,res)=>{
    ...
    const deletedSeller=await Seller.findByIdAndDelete(req.user._id);
    if(!deletedSeller){
        throw new ApiError(404,"Error in deleting seller")
    }
    if(sellerImage){
        try {
            await deleteFromCloudinary(sellerImage);
        } catch (error) {
            console.error('Error deleting image from Cloudinary:', error);
            throw new ApiError(500, "Error deleting image from Cloudinary");
        }
